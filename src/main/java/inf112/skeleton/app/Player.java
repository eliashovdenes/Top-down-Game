...
public class Player extends GameObject {
    ...
    private void update(float deltaTime) {
        ...
        if (controller.isFast()) {
            ...
            try {
                if (controller.isUp())
                    animate(PlayerAnimation.RUNUP.animation, deltaTime);
                else if (controller.isDown())
                    animate(PlayerAnimation.RUNDOWN.animation, deltaTime);
                if (controller.isLeft())
                    animate(PlayerAnimation.RUNLEFT.animation, deltaTime);
                else if (controller.isRight())
                    animate(PlayerAnimation.RUNRIGHT.animation, deltaTime);
        ...
        if (controller.isAttack()) {
            ...
            if (timer < 1) {
                if (playerAnimation == PlayerAnimation.DOWN.animation
                        || playerAnimation == PlayerAnimation.RUNDOWN.animation) {
                ...
                if (playerAnimation == PlayerAnimation.UP.animation
                        || playerAnimation == PlayerAnimation.RUNUP.animation) {
                ...
                if (playerAnimation == PlayerAnimation.LEFT.animation
                        || playerAnimation == PlayerAnimation.RUNLEFT.animation) {
                ...
                if (playerAnimation == PlayerAnimation.RIGHT.animation
                        || playerAnimation == PlayerAnimation.RUNRIGHT.animation) {
        ...
        if (controller.isShoot()) {
            if (shootTimer <= 0) {
                ...
                if (projVelX > 0 && projVelY == 0) {
                    texture = new Texture(PlayerPics.RIGHTARROW.source);
                }
                if (projVelX < 0 && projVelY == 0) {
                    texture = new Texture(PlayerPics.LEFTARROW.source);
                }
                if (projVelX == 0 && projVelY > 0) {
                    texture = new Texture(PlayerPics.UPARROW.source);
                }
                if (projVelX == 0 && projVelY < 0) {
                    texture = new Texture(PlayerPics.DOWNARROW.source);
                }
        ...
        // TODO må skrive en funskjon som holder følge på hvilke retning spilleren sist
        // beveget seg
    ...
    // changes the textureregion and initialises the animation, also updates the
    // frame.
